...
## OSTEOPOROSE_PRINT_PDF — 2026-02-06

### Contexto

Correções para PDF fallback: 1 slide por página, paleta completa no print, evitar “sliver” do próximo slide.

### Ajustes aplicados

- **scripts/build-osteoporose-print-html.js**
  - `@media print`: `.slide` com `page-break-after:always`, `overflow:hidden`, `min-height`/`max-height` 9.375in, `box-sizing:border-box`; primeiro/último slide com exceção de page-break.
  - `:root` do print: adicionados `--gold-dark`, `--gold-dark-rgb`, `--font-serif` (paleta e fontes alinhadas ao base.css).
  - `.slides-container` em print: `margin:0`, `overflow:hidden`. `.slide` em print: `padding:0`, `isolation:isolate`, `print-color-adjust:exact`.
- **OSTEOPOROSE/src/css/print.css**
  - `@media screen` + `html.is-print .slide`: caixa fixa 16:9 (1600×900px) para pré-visualização ?print=1 (1 slide por “página” no scroll).

### Arquivos modificados

- `scripts/build-osteoporose-print-html.js`
- `OSTEOPOROSE/src/css/print.css`
- `OSTEOPOROSE/src/print.html` (regenerado pelo build)

### Constraints

- HC2: alterações em print.css dentro do limite (caixa 16:9 ~6 linhas).
- HC3: !important apenas em print.css (exceção permitida) e no CSS gerado pelo build (saída de print).
- Erro 9 (CLAUDE.md): `overflow:hidden` em html/body no @media print mantido no build script.
- Pipeline: apenas build script editado; print.html não editado à mão.

### Reversão (mesma sessão)

- Estratégia scale(0.92) + wrapper `.slide__print-inner` desfeita: piorou o PDF. Voltou ao estado anterior: 1 slide por página, overflow hidden (conteúdo que passar de 9.375in é cortado; sem scale).
...
- Slide 03: evita “boxes” esticados (alinha o bloco principal em `align-items: start`) para reduzir espaço vazio _dentro_ dos cards.
