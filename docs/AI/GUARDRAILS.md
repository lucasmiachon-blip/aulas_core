...
# PARTE 9: HARD CONSTRAINTS (INVIOLÁVEIS)

> **Estas regras são ABSOLUTAS. Não há exceções sem aprovação explícita do usuário.**

## 🔴 HC1: DOCUMENTAÇÃO OBRIGATÓRIA

```
TODA mudança em código DEVE incluir:
├── CHANGELOG.md (raiz) → O que mudou, por quê
├── README.md → Se estrutura/arquitetura mudar
└── [PROJETO]/CHANGELOG.md → Se mudança for específica do projeto
```

**CHECKLIST PÓS-CÓDIGO:**
- [ ] `CHANGELOG.md` atualizado?
- [ ] Mudança documentada com data?
- [ ] README precisa de atualização?

**VIOLAÇÃO = TAREFA INCOMPLETA**

## 🔴 HC2: CSS MÍNIMO

| Regra | Limite |
|-------|--------|
| Linhas de CSS por tarefa | **Máximo 5-10** |
| Criar nova classe | **Evitar** - usar existente |
| Criar novo arquivo CSS | **PROIBIDO** |
| Novos tokens em :root | **Preferir ajustar existentes** |

```
SE precisar de mais CSS:
1. PARE
...
/* ❌ NUNCA FAZER */
.minha-classe {
  display: flex !important;  /* PROIBIDO */
}

/* ✅ CORRETO */
.slide .minha-classe {
  display: flex;  /* Aumentar especificidade se necessário */
}
```

**Única exceção:** `print.css` para regras de impressão (já existentes).

**Se parecer necessário !important:**
1. A especificidade está errada
2. Refatorar seletores
3. Nunca resolver com !important

## 🔴 HC4: VERIFICAÇÃO PÓS-CSS

```powershell
# Rodar SEMPRE após mexer em CSS:
grep -c "!important" OSTEOPOROSE/src/css/*.css
grep -c "!important" GRADE/src/css/*.css

# Se o número AUMENTOU → REVERTER E REFAZER
```

---

*Guardrails criados em 30/01/2026*
*Atualizado: 2026-02-02 (Hard Constraints adicionados)*
*Versão 2.0*
