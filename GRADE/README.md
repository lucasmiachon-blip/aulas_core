...
## GRADE — Atualização (02/02/2026)

- A sequência principal fecha em **Camões** (slide 36).
- **S36 (SBC 2025: CAC reclassifica risco → meta LDL-c)** foi incorporado no bloco CAC (logo após S09).
- **S12 (CAC=0 não exclui placa não calcificada)** foi movido para o apêndice.
- O hash da URL segue o **contador do slide** (ex.: `#9`), mantendo suporte a `#Sxx`.
- Estrutura unificada (sem duplicação `GRADE/GRADE`).
- Tipografia: títulos (`h1/h2/h3`) agora mantêm **Georgia** mesmo quando há `<span>/<em>` dentro do título (corrige “peso” diferente em trechos destacados).
- **S09 (CAC)** agora inclui as **recomendações ipsis litteris** da SBC 2025 (com força/certidão), mantendo o padrão visual do bloco de bempedóico.
...
- **Corpo:** Inter (sans-serif) — legibilidade premium
...
## GRADE — PATCH 2.11 (2026-02-01)

Correções solicitadas após revisão visual (P1 — **slides 1–36**):

- **Reordenação do viewer:** `S52` → posição **16**; `S51` (tabela benchmark) → posição **31**.
- **S52:** tabela ajustada para ficar no **padrão estético do S51**; removido desalinhamento causado por `border-top`; refinada hierarquia tipográfica.
- **S17–S20:** régua/IC e cards com UI mais limpa (ticks alinhados, menos “peso” de borda, melhor contraste).
- **S60/S61:** cards agora alinham topo/base e rodapé padronizado (sem `position:absolute`).
- **Tipografia global:** `base.css` ganhou classes (`eyebrow/title/subtitle/ref`) para uniformizar header e footer; e vários cards tiveram `border: 2px` reduzido para `1px` para harmonia.

> Nesta etapa **não** há mudança de conteúdo, e **não** foi iniciada a etapa de atenção/remoção de ruído/interatividade.



## GRADE — PATCH 2.12 (2026-02-02)

Início do **P2 (conteúdo + forma)** — batch 1 nos **slides 1–10 (viewer)**, aplicando boas práticas (Duarte, hierarquia atencional, chunking/sinalização) com o objetivo de **melhor retenção** e **menor carga cognitiva**.

- **S01–S02:** abertura com tese explícita + números em card (mais “escaneável”).
- **S03:** CAC como caso‑âncora (fluxo risco → meta → benefício absoluto) + gramática do GRADE.
- **S04:** interação reformatada para uso em sala (QR + instrução curta).
- **S05–S06:** divisor (certeza ≠ força) e motor do GRADE com stepper e listas compactas.
- **S60/S09/S36/S10:** início do bloco CAC com roteiro, aplicação e tabela de metas; CAC=0 com “warranty” em duas janelas (3–7 / 5–10 anos).

Docs atualizados: **CHANGELOG** e **README**.

> Neste batch, mantivemos a **ordem** e evitamos mexer nos slides fora do recorte 1–10.
